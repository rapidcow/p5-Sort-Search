...
C<$lo> is the index of the leftmost/first array element and
C<$hi> is one more than the rightmost/final array element.
Notably, C<$hi> is the fallback index when search finds a
bisection point to the right of the whole array.

=item *

For right searches (subroutines containing the letter "r"),
C<$lo> is one less than the index of the leftmost/first array
element and C<$hi> is the rightmost/final array element.
Notably, C<$lo> is the fallback index when search finds a
...
In list context, C<blsect2> returns both C<$lower> and C<$upper_x>,
