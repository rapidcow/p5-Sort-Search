...
B<NOTE>:
This is an intentionally terse technical document.
...
and C<< $cmp > 0 >> represents C<< $_ > $target >>.  When the list is
sorted in increasing order, C<< $_ <=> $target >> is a valid ordering:
...
The return value of B<blsrch0> is the unique index C<$lower> on
...
The return value of B<blsrch1> is the unique index C<$upper_x>
...
(Compare this to B<brsrch0>.)

=back

In list context, B<blsrch2> returns both C<$lower> and C<$upper_x>,
...
represents C<< $_ < $target >>; C<$cmp == 0> represents C<$_ == $target>;
and C<< $cmp > 0 >> represents C<< $_ > $target >>.  When the list is
sorted in increasing order, C<< $target <=> $_ >> is a valid ordering:
...
The return value of B<brsrch0> is the unique index C<$upper> on
the closed interval C<[$lo, $hi]> such that:
...
The return value of B<brsrch1> is the unique index C<$lower_x> on
the closed interval C<[$lo, $hi]> such that:
...
(Compare this to B<blsrch0>.)

=back

In list context, B<brsrch2> returns both C<$upper> and C<$lower_x>,
in that order.  In scalar context, it returns the nonnegative
...
More about this distribution can be found at:
