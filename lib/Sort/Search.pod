...
For B<bixectr>, it's the same against B<bisectr> except
...
The return value of B<blsect0> is the unique index C<$lower> on
...
(Compare this to B<brsect1>.)

=back

The return value of B<blsectx> is any index C<$x> satisfying
C<< $ord->($x) == 0 >>, but if one could not be found,
it returns the same result as C<blsect0>.

The return value of B<blsect1> is the unique index C<$upper_x>
...
(Compare this to B<brsect0>.)

=back

In list context, B<blsect2> returns both C<$lower> and C<$upper_x>,
...
The return value of B<brsect0> is the unique index C<$upper> on
the closed interval C<[$lo, $hi]> such that:
...
(Compare this to B<blsect1>.)

=back

The return value of B<brsectx> is any index C<$x> satisfying
C<< $ord->($x) == 0 >>, but if one could not be found,
it returns the same result as C<brsect0>.

The return value of B<brsect1> is the unique index C<$lower_x> on
the closed interval C<[$lo, $hi]> such that:
...
(Compare this to B<blsect0>.)

=back

In list context, B<brsect2> returns both C<$upper> and C<$lower_x>,
in that order.  In scalar context, it returns the nonnegative
