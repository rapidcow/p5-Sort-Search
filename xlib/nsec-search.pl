...
sub _bsrchpx
{
	...
	if (defined $res && $res == 0) {
		...
		--$hi;
		if ($lo == $hi) {
			# SPECIAL: The search range has almost
			# collapsed, even though we have found
			# the (sole) match.  This would be our
			# common pointer, so return that.
			$prv = $nxt = $elp;
		} else {
			# Search on a possibly long range?
